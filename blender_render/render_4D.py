...
# "C:\Program Files (x86)\Steam\steamapps\common\Blender\blender.exe" --background --python "C:\Users\Philips Deng\Documents\GitHub\Scriptsfor24WS\blender_render\render_4D.py"
...
base_path = r"C:\Users\Philips Deng\Documents\GitHub\Scriptsfor24WS\data\deer"

all_files = os.listdir(base_path)


obj_files = sorted([f for f in all_files if f.endswith('.obj')], key=lambda x: int(re.search(r'\d+', x).group()))
textures = sorted([f for f in all_files if f.endswith('.png')])
...
def create_area_light(name, location, rotation_euler, energy=30000, size=100):
    """
...
# camera_object.location = (0, -18, 10)
camera_object.location = (0, -6, 3)
camera_object.rotation_euler = (math.radians(75), 0, 0) #越高看的越往上
camera_data.lens = 50
